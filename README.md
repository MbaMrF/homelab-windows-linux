...
| Componente | Detalle |
| --- | --- |
| **Dominio** | `fructuoso.local` con DC Windows Server 2022 |
...
- **hMailServer**: Dominio `fructuoso.local` con cuentas de prueba
...
New-ADOrganizationalUnit -Name "Ventas" -Path "DC=fructuoso,DC=local"
New-ADUser -Name "j.perez" -Path "OU=Ventas,DC=fructuoso,DC=local" -Enabled $true
